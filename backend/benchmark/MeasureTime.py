import time
from app.utils.Colors import BHWHITE, MAGB, RESET
...
class MeasureTime:
    ...
    def stop(self, get_str: bool = False, duration_only: bool = False):
        if self.t_start is None:
            ...
            self.t_end = time.time() - self.t_start
            self.t_start = None
            if duration_only:
                # string_time = f"{self.t_end // 1000000}ms"
                string_time = f"{self.t_end:.6f}s"
            else:
                # string_time = f"{BHWHITE}Duration : {MAGB} {BHWHITE}{self.t_end // 1000000}ms {RESET}"
                string_time = f"{BHWHITE}Duration : {MAGB} {BHWHITE}{self.t_end:.6f}s {RESET}"
            if get_str:
                return string_time
            else:
                print(string_time)
